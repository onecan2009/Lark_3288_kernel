...
static int rk3288_lcdc_set_dclk(struct rk_lcdc_driver *dev_drv)
{
	...
	struct lcdc_device *lcdc_dev =
	    container_of(dev_drv, struct lcdc_device, driver);
	struct rk_screen *screen = dev_drv->cur_screen;
    printk("begin set rate......\n");
	ret = clk_set_rate(lcdc_dev->dclk, screen->mode.pixclock);
    printk("over set rate......\n");
	if (ret)
		dev_err(dev_drv->dev, "set lcdc%d dclk failed\n", lcdc_dev->id);
#ifdef SET_PIX_FLAG
    printk("screen->mode.pixclock is %u\n",screen->mode.pixclock);
    
    lcdc_dev->pixclock = 11695;
	...
	lcdc_dev->driver.pixclock = lcdc_dev->pixclock;
	printk("lcdc_dev->dclk is %lu\n",clk_get_rate(lcdc_dev->dclk));
    printk("lcdc_dev->pixclock is %lu\n",lcdc_dev->pixclock);
