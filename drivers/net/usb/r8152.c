...
u8 usb_mac_defined[8]={0x00,0x90,0x4c,0x49,0x08,0x05,0x0,0x0};
...
//add to setup macaddress of usblan @oc
//date to mac buff
user_va=(char *)kmalloc(18,GFP_KERNEL);
user_usb_va=(char *)kmalloc(18,GFP_KERNEL);
if( s && user_va && user_usb_va)
{
strpar(s,user_usb_va,user_va);
}
else 
{
	strncpy(user_usb_va,"00:90:4c:11:34:e3",18);
}
	printk("this new test kernel@sampson\n");
	printk("user_usb_va is %s,the func is %s\n",user_usb_va,__func__);
		
			p=strsep(&user_usb_va,":");
		    while(p)
           {
             	usb_mac_defined[a]=strtol(p,NULL,16);
              	p=strsep(&user_usb_va,":");
              	a++;
        	}
	if (pla_ocp_read(tp, PLA_IDR, sizeof(u8) * 8, node_id) < 0)
		...
		for(a=0;a<sizeof(u8)*6;a++){
			node_id[a]=usb_mac_defined[a];
			}
	for(a=0;a<sizeof(u8)*8;a++)
		printk("node_id[%d] is %02x\n",a,node_id[a]);
		memcpy(dev->dev_addr, node_id, dev->addr_len);
		memcpy(dev->perm_addr, dev->dev_addr, dev->addr_len);

	
	ocp_write_byte(tp, MCU_TYPE_PLA, PLA_CRWECR, CRWECR_CONFIG);
	pla_ocp_write(tp, PLA_IDR, BYTE_EN_SIX_BYTES, 8, usb_mac_defined);
...
//	printk("IN %s\n",__func__);
	...
	for(i=0;i<14;i++)
	{
			printk(" the addr->sa_data[%d] is %x\n",i,addr->sa_data[i]);
		}
...
{
	//	printk("IN %s,the netdev is %p,the ethtool_cmd is %d\n",__func__,netdev,cmd->phy_address);
	struct r8152 *tp = netdev_priv(netdev);
...
printk("version  is %d,the func is %s\n",version,__func__);
	switch (version) {
	case 0x4c00:
